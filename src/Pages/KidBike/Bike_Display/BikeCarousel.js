...
const BikeCarousel = ({ bikes = [], labels = [], onBikeSelect }) => {
  ...
  const fourthBike = bikes[(currentIndex + 3) % bikes.length]; // Fourth bike
  ...
  return (
    <div className="bike-carousel">
      ...
      >
        ...
        {/* Fourth bike item */}
        <div className="carousel-bike">
          <div className="bike-item" onClick={() => onBikeSelect(fourthBike)}>
            <img src={fourthBike.image} alt={fourthBike.name} className="carousel-image" />
            {isEditing ? (
              <input
                type="text"
                value={editableLabels[currentIndex + 2] || fourthBike.name}
                onChange={(e) => {
                  const newLabels = [...editableLabels];
                  newLabels[currentIndex + 2] = e.target.value;
                  setEditableLabels(newLabels);
                }}
                onBlur={() => handleSaveLabel(currentIndex + 2)}
                autoFocus
              />
            ) : (
              <p className="bike-label" onClick={() => handleEditLabel(currentIndex + 2)}>
                {editableLabels[currentIndex + 2] || fourthBike.name}
              </p>
            )}
          </div>
        </div>
